...
- `agents/public/*.md` — öffentliche Agentenrollen,
- `agents/manifest.yaml` — Rollen-/Routingmanifest,
- `skills/*/SKILL.md` — wiederholbare Skills,
- `evals/quality-gates.md` — Qualitäts- und Sicherheitsgates.
...
Lies AGENTS.md, CLAUDE.md, README.md, agents/manifest.yaml und evals/quality-gates.md.
...
| Agenten/Subagents prüfen | `/agents` | Fachlogik bleibt in `agents/public/*.md`, nicht nur in Claude-Konfiguration. |
...
2. Passende Rolle aus `agents/public/role-model.md` wählen.
3. Relevantes Profil vollständig lesen.
4. Bei wiederholbarer Aufgabe passenden Skill aus `skills/` lesen.
5. Plan erstellen.
...
Lies zuerst AGENTS.md, CLAUDE.md, agents/public/role-model.md und das passende Profil unter agents/public/.
Wenn ein Skill passt, lies zusätzlich skills/<skill>/SKILL.md.
