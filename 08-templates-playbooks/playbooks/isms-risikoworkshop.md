...
For individual risks or small workshops, the risk process can be carried out as a guided wizard. The wizard uses the skill `isms-risk-analysis` (`../../07-ai-governance-agents/skills/isms-risk-analysis/SKILL.md`) and records the status in `templates/risikoanalyse-session-state.md` (`../templates/risikoanalyse-session-state.md`).
