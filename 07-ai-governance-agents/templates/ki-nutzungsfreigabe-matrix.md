...
If `local AI` or a local redaction tool is selected, `01-orientation/setup/lokale-ki.md` must also be reviewed. Model/checkpoint approval, Windows pilot, storage locations, logging, network binding, team use, evaluation, and offboarding are clarified there. The summary overview is additionally available in `01-orientation/setup/ki-setups-bedienungsanleitung.md`.
