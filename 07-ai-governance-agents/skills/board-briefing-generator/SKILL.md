---
name: board-briefing-generator
version: 1.0.0
description: "Generate board briefings from risks, evidence, options, decisions, and next steps."
category: management
inputs:
  - scope
  - existing-artifacts
...
This skill supports user organizations by generating a decision-oriented board/management brief from reviewed risks, evidence, options, and open decisions.
...
- board briefings, management decisions, or escalations from governance routines,
...
- **Existing artifacts:** available registers, templates, playbooks, measures, evidence, or role information.
...
- `08-templates-playbooks/templates/board-briefing-onepager.md`, once available,
...
3. **Derive operating logic**  
   Define owner, trigger, sequence, output, evidence, review cadence, and escalation point.
...
7. **Prepare review or brief**  
   Condense results into a review artifact with open decisions, options, risks of inaction, and the next step.
...
- to `management-review-facilitator` when subject-matter design needs to be deepened,
...
A fictional organization wants to address a single governance topic in a structured way. There are initial notes, but no clear routine, no reliable evidence, and no prioritized decision template.

Good skill output:

- Scope and non-scope are named.
- A proposed owner and review role are visible.
- The routine has a trigger, sequence, output, and evidence.
- Open decisions are marked in the decision brief.
- The next measure is small enough for the next review cycle.

Poor skill output:

- “The topic is complete as soon as a policy has been written.”
