...
The public user agents are located under [`agents/public/`](public/). They form a role model for organizations that want to build their own compliance and security governance agent system with krisensicherOS.
...
For getting started with a small number of roles, see [`agents/public/anwender-routing.md`](public/anwender-routing.md). The complete role model is in [`agents/public/role-model.md`](public/role-model.md), and the routing manifest is in [`agents/manifest.yaml`](manifest.yaml).
